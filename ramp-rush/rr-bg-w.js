(function () {
  ...
  function updateHud() {
    ...
    if (speedLabelEl && (!running || paused || dead || falling)) {
      speedLabelEl.textContent = `${(speed / 14).toFixed(1)}×`;
    }
  }
  ...
  function beginFall(reason, opts = {}) {
    ...
    if (opts.fling !== false) {
      ...
      ballVX = side * Math.max(8, Math.abs(ballVX) + 5);
      if (ballVH < 2.5) ballVH = 3.2;
    }
  ...
  function updateFall(dt) {
    fallTimer += dt;
    ballVX *= Math.pow(0.55, dt);
    ballX += ballVX * dt;
    ballVH -= AIR_GRAVITY * 1.2 * dt;
    ballH += ballVH * dt;
    // Keep drifting forward so the neon road slips away above you.
    worldZ += Math.max(4, speed * 0.4) * dt;
    ...
    if (fallTimer > 1.55 || ballH < -20 || camFollowY < -16) {
  ...
  function update(dt) {
    ...
    ballVX += steer * 40 * dt;
    ballVX *= Math.pow(0.1, dt);
    ballX += ballVX * dt;
    updateCamera(dt);

    // Gravity builds baseline speed; carving sideways raises the ceiling hard.
    const carve = Math.min(1.3, Math.abs(ballVX) / 6.5);
    const baseTarget = Math.min(36, 11 + score * 0.048);
    speed += (baseTarget - speed) * Math.min(1, GRAVITY_ACCEL * dt);
    speed += HILL_SLOPE * 5.5 * dt;
    if (carve > 0.08) speed += carve * 28 * dt;
    const maxSpeed = 36 + carve * 26;
    if (speed > maxSpeed) speed = maxSpeed;
    // Extra travel while sliding sideways so the boost is obvious.
    const travel = speed * (1 + carve * 0.55);
    worldZ += travel * dt;
    score += travel * dt * 0.55;
    if (speedLabelEl) speedLabelEl.textContent = `${(travel / 14).toFixed(1)}×`;
    fillAhead();
  ...
  function drawBackground() {
    // Slope-style neon night void.
    const sky = ctx.createLinearGradient(0, 0, 0, H);
    sky.addColorStop(0, "#02040a");
    sky.addColorStop(0.4, "#060b18");
    sky.addColorStop(0.7, "#0a1224");
    sky.addColorStop(1, "#101a30");
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, W, H);

    const glow = ctx.createRadialGradient(W * 0.5, HORIZON + 10, 2, W * 0.5, HORIZON + 10, W * 0.5);
    glow.addColorStop(0, "rgba(40, 220, 255, 0.2)");
    glow.addColorStop(0.35, "rgba(80, 40, 180, 0.1)");
    glow.addColorStop(1, "rgba(0, 0, 0, 0)");
    ctx.fillStyle = glow;
    ctx.fillRect(0, 0, W, H);

    for (let i = 0; i < 60; i += 1) {
      const sx = ((i * 89 + worldZ * 0.5) % W + W) % W;
      const sy = ((i * 47) % Math.max(8, HORIZON - 6)) + 3;
      ctx.fillStyle = "rgba(200,230,255," + (0.12 + (i % 5) * 0.05) + ")";
      ctx.fillRect(sx, sy, 1.2, 1.2);
    }

    // Far neon city flecks
    for (let i = 0; i < 24; i += 1) {
      const lx = W * 0.15 + i * (W * 0.03) + Math.sin(worldZ * 0.02 + i) * 3;
      const ly = HORIZON + 4 + (i % 3);
      const cyan = i % 3 !== 1;
      ctx.fillStyle = cyan ? "rgba(80, 240, 255, 0.55)" : "rgba(255, 70, 180, 0.45)";
      ctx.fillRect(lx, ly, 2.2, 2.2);
    }

    const streakCount = running ? 36 : 14;
    for (let i = 0; i < streakCount; i += 1) {
      const y = HORIZON + 24 + ((i * 41 + worldZ * 10) % (H - HORIZON - 50));
      const len = 22 + (i % 6) * 12;
      const x = ((i * 109 - worldZ * (running ? 55 : 10)) % (W + len) + W + len) % (W + len) - len;
      const cyan = i % 4 !== 2;
      ctx.strokeStyle = cyan
        ? "rgba(60, 230, 255," + (0.06 + (i % 3) * 0.03) + ")"
        : "rgba(255, 60, 170," + (0.05 + (i % 3) * 0.025) + ")";
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + len, y);
    ...
    haze.addColorStop(0, "rgba(10, 20, 40, 0)");
    haze.addColorStop(0.55, "rgba(4, 10, 22, 0.3)");
  ...
  function drawSegment(seg) {
    ...
    const ramp = !!seg.ramp;
    if (ramp) {
      ctx.fillStyle = `rgba(${Math.floor((40 + (seg.stripe ? 8 : 0)) * shade)}, ${Math.floor((70 + (seg.stripe ? 10 : 0)) * shade)}, ${Math.floor((90 + (seg.stripe ? 12 : 0)) * shade)}, 1)`;
    } else {
    ...
    // Neon cyan edge rails on the road
    const edgeCore = ramp ? "rgba(255, 210, 90, 0.85)" : "rgba(70, 245, 255, 0.85)";
    const edgeGlow = ramp ? "rgba(255, 170, 40, 0.22)" : "rgba(40, 200, 255, 0.25)";
