(function () {
  ...
  function update(dt) {
    ...
    // Sideways cuts add forward speed (Slope-style carve boost).
    const sideSpeed = Math.min(1, Math.abs(ballVX) / 9);
    const carve = Math.max(sideSpeed, Math.abs(steer) * 0.55);
    const targetSpeed = Math.min(56, 12 + score * 0.055 + carve * 10);
    speed += (targetSpeed - speed) * Math.min(1, GRAVITY_ACCEL * dt);
    speed += HILL_SLOPE * 10 * dt;
    speed += carve * 14 * dt;
    if (speed > 56) speed = 56;
    worldZ += speed * dt;
    score += speed * dt * 0.55;
    fillAhead();
