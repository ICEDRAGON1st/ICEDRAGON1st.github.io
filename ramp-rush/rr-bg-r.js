(function () {
  ...
  function drawBackground() {
    ...
    for (let i = 0; i < 28; i += 1) {
      const lx = W * 0.12 + i * (W * 0.028) + Math.sin(worldZ * 0.015 + i) * 4;
      const ly = HORIZON + 6 + (i % 4) * 1.5;
      const warm = i % 3 !== 1;
      ctx.fillStyle = warm ? "rgba(255, 200, 120, 0.55)" : "rgba(170, 210, 255, 0.45)";
      ctx.fillRect(lx, ly, 2.5, 2.5);
      ctx.beginPath();
      ctx.arc(lx + 1, ly + 1, 4 + (i % 3), 0, Math.PI * 2);
      ctx.fillStyle = warm ? "rgba(255, 180, 90, 0.08)" : "rgba(140, 180, 255, 0.07)";
      ctx.fill();
    }
  ...
  function drawDescentScenery() {
    ...
    for (let z = Math.floor((worldZ + 4) / lampStep) * lampStep; z < worldZ + 75; z += lampStep) {
      ...
      const x = side * (TRACK_HALF + 0.55);
      const poleTop = 2.35;
      rockQuad(
        x - 0.05, -0.1, z,
        x + 0.05, -0.1, z,
        x + 0.05, poleTop, z + 0.15,
        x - 0.05, poleTop, z + 0.15,
        "rgba(" + Math.floor(45 * fade) + "," + Math.floor(48 * fade) + "," + Math.floor(54 * fade) + ",0.95)",
        null
      );
      rockQuad(
        x, poleTop - 0.08, z,
        x - side * 0.85, poleTop + 0.05, z,
        x - side * 0.85, poleTop + 0.18, z + 0.12,
        x, poleTop + 0.05, z + 0.12,
        "rgba(" + Math.floor(50 * fade) + "," + Math.floor(54 * fade) + "," + Math.floor(60 * fade) + ",0.95)",
        null
      );

      const bulbX = x - side * 0.7;
      const bulbY = poleTop + 0.05;
      const bulb = project(bulbX, bulbY, z + 0.06);
      const pool = project(bulbX - side * 0.2, 0.02, z + 0.4);
      if (bulb) {
        const r = Math.max(3, bulb.scale * 0.22);
        const halo = ctx.createRadialGradient(bulb.x, bulb.y, 0, bulb.x, bulb.y, r * 4.5);
        halo.addColorStop(0, "rgba(255, 230, 170," + (0.35 * fade) + ")");
        halo.addColorStop(0.35, "rgba(255, 190, 100," + (0.12 * fade) + ")");
        halo.addColorStop(1, "rgba(255, 160, 60, 0)");
        ctx.fillStyle = halo;
        ctx.beginPath();
        ctx.arc(bulb.x, bulb.y, r * 4.5, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
      ...
      if (pool && bulb) {
        const left = project(bulbX - 1.1, 0.02, z + 1.1);
        const right = project(bulbX + 1.1, 0.02, z + 1.1);
        if (left && right) {
          ctx.beginPath();
          ctx.moveTo(bulb.x, bulb.y);
          ctx.lineTo(left.x, left.y);
          ctx.lineTo(right.x, right.y);
          ctx.closePath();
          const cone = ctx.createLinearGradient(bulb.x, bulb.y, pool.x, pool.y);
          cone.addColorStop(0, "rgba(255, 210, 140," + (0.14 * fade) + ")");
          cone.addColorStop(1, "rgba(255, 180, 90," + (0.03 * fade) + ")");
          ctx.fillStyle = cone;
          ctx.fill();
        }
        const pr = Math.max(8, pool.scale * 1.4);
        const puddle = ctx.createRadialGradient(pool.x, pool.y, 0, pool.x, pool.y, pr);
        puddle.addColorStop(0, "rgba(255, 220, 150," + (0.22 * fade) + ")");
        puddle.addColorStop(0.45, "rgba(255, 190, 110," + (0.08 * fade) + ")");
        puddle.addColorStop(1, "rgba(255, 160, 80, 0)");
        ctx.fillStyle = puddle;
        ctx.beginPath();
        ctx.ellipse(pool.x, pool.y, pr, pr * 0.35, 0, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ...
    fog.addColorStop(0.7, "rgba(4, 6, 10, 0.1)");
