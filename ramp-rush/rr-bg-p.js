(function () {
  ...
  function drawDescentScenery() {
    ...
    for (const side of [-1, 1]) {
      ...
      // Solid opaque masses first (stable — no strip z-fighting).
      fillRibbon(
        samples.map((s) => s.pTo),
        samples.map((s) => s.pBo),
        shadeRock([48, 38, 30], 0.82, lit)
      );
      ...
      fillRibbon(topsIn, botsIn, shadeRock(lit ? [118, 92, 68] : [96, 76, 58], 1, lit));
      ...
      ctx.closePath();
      ctx.fillStyle = shadeRock(lit ? [132, 104, 78] : [108, 86, 66], 0.92, lit);
      ctx.fill();

      // Light rock detail clipped to the inner face only.
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(topsIn[0].x, topsIn[0].y);
      for (let i = 1; i < topsIn.length; i += 1) ctx.lineTo(topsIn[i].x, topsIn[i].y);
      ...
      for (let band = 0; band < 5; band += 1) {
        const uBase = 0.18 + band * 0.14;
        ctx.beginPath();
        let started = false;
        for (let i = 0; i < samples.length; i += 1) {
          const s = samples[i];
          const wobble = (rockNoise(s.z * 0.25, band) - 0.5) * 0.06;
          const u = Math.max(0.08, Math.min(0.9, uBase + wobble));
          const x = s.pTi.x * (1 - u) + s.pBi.x * u;
          const y = s.pTi.y * (1 - u) + s.pBi.y * u + (rockNoise(band, i) - 0.5) * 1.5;
          if (!started) {
            ctx.moveTo(x, y);
            started = true;
          } else ctx.lineTo(x, y);
        }
        ctx.strokeStyle = "rgba(20, 14, 10," + (0.1 + (band % 2) * 0.05) + ")";
        ctx.lineWidth = 1;
        ctx.stroke();
      }

      for (let g = 0; g < 120; g += 1) {
        const si = Math.floor(rockNoise(g * 1.9, side + 1) * (samples.length - 1));
        const s = samples[si];
        const u = rockNoise(g, s.z * 0.09);
        const x = s.pTi.x * (1 - u) + s.pBi.x * u + (rockNoise(g, 3) - 0.5) * 4;
        const y = s.pTi.y * (1 - u) + s.pBi.y * u + (rockNoise(4, g) - 0.5) * 3;
        ctx.fillStyle =
          rockNoise(g, 7) > 0.5
            ? "rgba(160, 135, 105, 0.07)"
            : "rgba(15, 11, 8, 0.09)";
      ...
      ctx.moveTo(topsIn[0].x, topsIn[0].y);
      for (let i = 1; i < topsIn.length; i += 1) ctx.lineTo(topsIn[i].x, topsIn[i].y);
      ctx.strokeStyle = lit ? "rgba(160, 135, 105, 0.14)" : "rgba(120, 100, 80, 0.1)";
      ctx.lineWidth = 1.3;
      ctx.stroke();
    }
    ...
    fog.addColorStop(0.65, "rgba(10, 9, 8, 0.14)");
