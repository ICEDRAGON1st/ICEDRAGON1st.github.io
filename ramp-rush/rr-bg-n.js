(function () {
  ...
  function drawDescentScenery() {
    ...
    for (const side of [-1, 1]) {
      ...
      ctx.moveTo(topsIn[0].x, topsIn[0].y);
      for (let i = 1; i < topsIn.length; i += 1) ctx.lineTo(topsIn[i].x, topsIn[i].y);
      ctx.strokeStyle = lit ? "rgba(190, 160, 120, 0.22)" : "rgba(150, 125, 95, 0.14)";
      ctx.lineWidth = 1.6;
      ctx.stroke();

      for (let band = 0; band < 6; band += 1) {
        const u = 0.14 + band * 0.12;
        ctx.beginPath();
        let started = false;
        for (let i = 0; i < topsIn.length; i += 1) {
          const x = topsIn[i].x * (1 - u) + botsIn[i].x * u;
          const y = topsIn[i].y * (1 - u) + botsIn[i].y * u + Math.sin(i * 0.55 + band) * 0.8;
          if (!started) {
            ctx.moveTo(x, y);
            started = true;
          } else ctx.lineTo(x, y);
        }
        ctx.strokeStyle = "rgba(40, 28, 18," + (0.12 + (band % 3) * 0.04) + ")";
        ctx.lineWidth = 1.1;
        ctx.stroke();
      }
    }
