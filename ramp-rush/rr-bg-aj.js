(function () {
  ...
  function ptsToSegs(pts) {
    // Slope-style: 1 point ≈ 1 track section.
  ...
  function resetWorld() {
    ...
    // Slope-style: ~20 sections before the first hazard, then every 15 points.
    const firstHazardScore = 20;
    const safeCount = Math.max(3, firstHazardScore);
    for (let i = 0; i < safeCount; i += 1) {
      segments.push(makeSegment(nextSegZ, 0));
      nextSegZ += SEGMENT_LEN;
    }

    // First encounter around the 20-point mark, then every 15 points.
  ...
  function update(dt) {
    ...
    speed = BASE_SPEED;
    worldZ += travel * dt;
    // Slope scoring: +1 per track section cleared.
    score = Math.floor((worldZ + PLAYER_Z) / SEGMENT_LEN);
    if (speedLabelEl) speedLabelEl.textContent = "1.0×";
