(function () {
  ...
  function hazardGapSegs() {
    return ptsToSegs(rand(15, 30.01));
  }
  ...
  function enqueueArch() {
    const roofH = rand(2.15, 2.55);
    const lane = 1.35;
    const openHalf = 2.05;
    // Side lanes climb onto the roof; center stays low to go through.
    pendingSegs.push({
      archApproach: { h0: 0, h1: roofH * 0.45, lane },
      raise: 0
    });
    pendingSegs.push({
      archApproach: { h0: roofH * 0.45, h1: roofH, lane },
      raise: 0
    });
    pendingSegs.push({ arch: { roofH, openHalf, thick: 0.55 }, raise: 0 });
    pendingSegs.push({ arch: { roofH, openHalf, thick: 0.55 }, raise: 0 });
    pendingSegs.push({
      archApproach: { h0: roofH, h1: 0, lane },
      raise: 0
    });
    featureCooldown = 5 + hazardGapSegs();
  }
  ...
  function makeSegment(z, difficulty) {
    ...
    if (pendingSegs.length) {
      ...
      if (difficulty > 0.7 && pick < 0.28) enqueueArch();
      else if (difficulty > 0.85 && pick < 0.55) enqueueJump(difficulty);
  ...
  function resetWorld() {
    ...
    // First encounter around the 20-point mark, then 15–30 points between patterns.
    const first = Math.random();
    if (first < 0.34) enqueueArch();
    else if (first < 0.62) enqueueJump(0.9);
  ...
  function surfaceHeight(z) {
    ...
    if (seg.archApproach) {
      ...
      // Outer lanes climb onto / off the arch roof; center stays on the road
      // unless you're already up on the roof and riding down.
      if (Math.abs(ballX) >= seg.archApproach.lane || ballH >= Math.max(seg.archApproach.h0, seg.archApproach.h1) - 0.5) {
        return h;
      }
      return 0;
    }
    if (seg.arch) {
      // Already up high → ride the roof deck; otherwise drive through underneath.
      const top = seg.arch.roofH + (seg.arch.thick || 0.55);
      if (ballH >= seg.arch.roofH - 0.45) return top;
      return 0;
    }
    if (seg.ramp) {
      const t = Math.max(0, Math.min(1, (z - seg.z) / SEGMENT_LEN));
  ...
  function rampSlope(z) {
    const seg = segmentAt(z);
    if (!seg) return 0;
    if (seg.archApproach && (Math.abs(ballX) >= seg.archApproach.lane || ballH >= Math.max(seg.archApproach.h0, seg.archApproach.h1) - 0.5)) {
      return (seg.archApproach.h1 - seg.archApproach.h0) / SEGMENT_LEN;
    }
  ...
  function drawSlopeGridFace(corners, opts = {}) {
    if (!corners.every(Boolean)) return;
    const fill = opts.fill || "rgba(2, 6, 12, 0.96)";
    const line = opts.line || "rgba(40, 190, 255, 0.85)";
    const glow = opts.glow || "rgba(30, 140, 255, 0.18)";
    ...
    ctx.fillStyle = fill;
    ctx.fill();
    // Soft outer glow like Slope neon edges
    ctx.strokeStyle = glow;
    ctx.lineWidth = 3.2;
    ctx.stroke();
    ...
    ctx.strokeStyle = "rgba(50, 200, 255, 0.28)";
    ctx.lineWidth = 1;
  ...
  function drawArchApproach(seg, half) {
    ...
    for (const side of [-1, 1]) {
      const x0 = side * lane;
      const x1 = side * half;
      const a = project(Math.min(x0, x1), h0, z0);
      const b = project(Math.max(x0, x1), h0, z0);
      const c = project(Math.max(x0, x1), h1, z1);
      const d = project(Math.min(x0, x1), h1, z1);
      drawSlopeGridFace([a, b, c, d], { uDiv: 3, vDiv: 4 });
      // Side wall under the climb so it reads as a solid Slope block
      const w0 = project(x0, 0, z0);
      const w1 = project(x0, h0, z0);
      const w2 = project(x0, h1, z1);
      const w3 = project(x0, 0, z1);
      drawSlopeGridFace([w0, w1, w2, w3], {
        fill: "rgba(1, 4, 10, 0.94)",
        line: "rgba(35, 170, 255, 0.7)",
        uDiv: 2,
        vDiv: 3
      });
    }
  ...
  function drawArchTunnel(seg, half) {
    // Slope-style square tunnel: black slabs + dense blue neon grid.
    const z0 = seg.z;
    const z1 = seg.z + SEGMENT_LEN;
    const roof = seg.arch.roofH;
    const thick = seg.arch.thick || 0.45;
    const open = seg.arch.openHalf;
    const blue = {
      fill: "rgba(2, 6, 14, 0.97)",
      line: "rgba(45, 200, 255, 0.9)",
      glow: "rgba(25, 130, 255, 0.22)"
    };

    // Left / right outer walls (full height boxes)
    for (const side of [-1, 1]) {
      const xInner = side * open;
      const xOuter = side * half;
      // Front face
      drawSlopeGridFace(
        [
          project(Math.min(xInner, xOuter), 0, z0),
          project(Math.max(xInner, xOuter), 0, z0),
          project(Math.max(xInner, xOuter), roof + thick, z0),
          project(Math.min(xInner, xOuter), roof + thick, z0)
        ],
        { ...blue, uDiv: 3, vDiv: 5 }
      );
      // Back face
      drawSlopeGridFace(
        [
          project(Math.min(xInner, xOuter), 0, z1),
          project(Math.max(xInner, xOuter), 0, z1),
          project(Math.max(xInner, xOuter), roof + thick, z1),
          project(Math.min(xInner, xOuter), roof + thick, z1)
        ],
        { ...blue, uDiv: 3, vDiv: 5 }
      );
      // Inner vertical face (tunnel wall) — dark with blue grid + red killer stripe like Slope
      const i0 = project(xInner, 0.02, z0);
      const i1 = project(xInner, 0.02, z1);
      const i2 = project(xInner, roof, z1);
      const i3 = project(xInner, roof, z0);
      if (i0 && i1 && i2 && i3) {
        ctx.beginPath();
        ...
        ctx.closePath();
        ctx.fillStyle = "rgba(120, 24, 40, 0.82)";
        ctx.fill();
      ...
      drawSlopeGridFace(
        [
          project(xOuter, 0, z0),
          project(xOuter, 0, z1),
          project(xOuter, roof + thick, z1),
          project(xOuter, roof + thick, z0)
        ],
        { ...blue, uDiv: 4, vDiv: 5 }
      );
    }

    // Ceiling underside (inside tunnel)
    drawSlopeGridFace(
      [
        project(-open, roof, z0),
        project(open, roof, z0),
        project(open, roof, z1),
        project(-open, roof, z1)
      ],
      { ...blue, fill: "rgba(1, 5, 12, 0.95)", uDiv: 6, vDiv: 3 }
    );

    // Rideable roof top (Slope platform on top of tunnel)
    drawSlopeGridFace(
      [
        project(-half, roof + thick, z0),
        project(half, roof + thick, z0),
        project(half, roof + thick, z1),
        project(-half, roof + thick, z1)
      ],
      { ...blue, uDiv: 7, vDiv: 4 }
    );

    // Front portal frame — square opening like classic Slope tunnels
    drawSlopeGridFace(
      [
        project(-open, roof, z0),
        project(open, roof, z0),
        project(open, roof + thick, z0),
        project(-open, roof + thick, z0)
      ],
      { ...blue, uDiv: 6, vDiv: 2 }
    );
    // Bright portal outline
    const portal = [
      project(-open, 0.02, z0 + 0.08),
      project(-open, roof, z0 + 0.08),
      project(open, roof, z0 + 0.08),
      project(open, 0.02, z0 + 0.08)
    ];
    if (portal.every(Boolean)) {
      ...
      ctx.lineTo(portal[1].x, portal[1].y);
      ctx.lineTo(portal[2].x, portal[2].y);
      ctx.lineTo(portal[3].x, portal[3].y);
