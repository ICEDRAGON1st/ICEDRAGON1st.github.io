(function () {
  ...
  function enqueueArch() {
    ...
    pendingSegs.push({ arch: { roofH, openHalf, thick: 0.42 }, raise: 0 });
    pendingSegs.push({ arch: { roofH, openHalf, thick: 0.42 }, raise: 0 });
    pendingSegs.push({
  ...
  function surfaceHeight(z) {
    ...
    if (seg.arch) {
      // Already up high → ride the roof; otherwise drive through underneath.
      if (ballH >= seg.arch.roofH - 0.55) return seg.arch.roofH;
      return 0;
    }
    if (seg.ramp) {
      const t = Math.max(0, Math.min(1, (z - seg.z) / SEGMENT_LEN));
  ...
  function drawGreenGridQuad(corners, fill) {
    if (!corners.every(Boolean)) return;
    ctx.beginPath();
    ctx.moveTo(corners[0].x, corners[0].y);
    ctx.lineTo(corners[1].x, corners[1].y);
    ...
    ctx.lineTo(corners[3].x, corners[3].y);
    ctx.closePath();
    ctx.fillStyle = fill || "rgba(4, 10, 6, 0.92)";
    ctx.fill();
    ctx.strokeStyle = "rgba(70, 255, 110, 0.75)";
    ctx.lineWidth = 1.6;
    ctx.stroke();
  }
  ...
  function drawArchApproach(seg, half) {
    ...
    for (const side of [-1, 1]) {
      ...
      drawGreenGridQuad([a, b, c, d], "rgba(6, 14, 8, 0.9)");
      // Grid seams
      for (let i = 1; i <= 3; i += 1) {
        const t = i / 4;
        const hh = h0 + (h1 - h0) * t;
        const zz = z0 + (z1 - z0) * t;
        const p0 = project(Math.min(x0, x1), hh + 0.02, zz);
        const p1 = project(Math.max(x0, x1), hh + 0.02, zz);
        if (p0 && p1) {
          ctx.beginPath();
          ctx.moveTo(p0.x, p0.y);
          ctx.lineTo(p1.x, p1.y);
  ...
  function drawArchTunnel(seg, half) {
    ...
    const thick = seg.arch.thick || 0.4;
    const open = seg.arch.openHalf;
    // Side pillars
    for (const side of [-1, 1]) {
      const xInner = side * open;
      const xOuter = side * half;
      const a = project(Math.min(xInner, xOuter), 0, z0);
      const b = project(Math.max(xInner, xOuter), 0, z0);
      const c = project(Math.max(xInner, xOuter), roof, z0);
      const d = project(Math.min(xInner, xOuter), roof, z0);
      drawGreenGridQuad([a, b, c, d], "rgba(3, 8, 5, 0.95)");
      const a1 = project(Math.min(xInner, xOuter), 0, z1);
      const b1 = project(Math.max(xInner, xOuter), 0, z1);
      const c1 = project(Math.max(xInner, xOuter), roof, z1);
      const d1 = project(Math.min(xInner, xOuter), roof, z1);
      drawGreenGridQuad([a1, b1, c1, d1], "rgba(3, 8, 5, 0.95)");
      // Inner red wall face (Slope vibe)
      const i0 = project(xInner, 0.02, z0);
      const i1 = project(xInner, 0.02, z1);
      const i2 = project(xInner, roof - 0.05, z1);
      const i3 = project(xInner, roof - 0.05, z0);
      if (i0 && i1 && i2 && i3) {
        ctx.beginPath();
        ...
        ctx.closePath();
        ctx.fillStyle = "rgba(140, 28, 42, 0.88)";
        ctx.fill();
      }
    }
    // Roof deck (rideable top)
    const r0l = project(-half, roof, z0);
    const r0r = project(half, roof, z0);
    const r1l = project(-half, roof, z1);
    const r1r = project(half, roof, z1);
    drawGreenGridQuad([r0l, r0r, r1r, r1l], "rgba(5, 12, 7, 0.94)");
    for (let i = 0; i < 5; i += 1) {
      const zz = z0 + 0.35 + i * ((z1 - z0) / 5);
      const g0 = project(-half * 0.95, roof + 0.02, zz);
      const g1 = project(half * 0.95, roof + 0.02, zz);
      if (g0 && g1) {
        ctx.beginPath();
        ctx.moveTo(g0.x, g0.y);
        ctx.lineTo(g1.x, g1.y);
        ctx.strokeStyle = "rgba(70, 255, 120, 0.4)";
        ctx.lineWidth = 1.2;
        ctx.stroke();
      }
    }
    // Front arch curve (stepped semicircle look)
    const steps = 8;
    ctx.beginPath();
    let started = false;
    for (let i = 0; i <= steps; i += 1) {
      const ang = Math.PI - (i / steps) * Math.PI;
      const x = Math.cos(ang) * open;
      const y = Math.sin(ang) * (roof * 0.92);
      const p = project(x, Math.max(0.05, y), z0 + 0.15);
      if (!p) continue;
      if (!started) {
        ctx.moveTo(p.x, p.y);
        started = true;
    ...
    if (started) {
      ctx.strokeStyle = "rgba(90, 255, 130, 0.85)";
      ctx.lineWidth = 2.4;
      ctx.stroke();
    }
    // Roof thickness underside lip
    const u0l = project(-open, roof - thick, z0);
    const u0r = project(open, roof - thick, z0);
    const u1l = project(-open, roof, z0);
    const u1r = project(open, roof, z0);
    drawGreenGridQuad([u0l, u0r, u1r, u1l], "rgba(8, 18, 10, 0.85)");
  }
